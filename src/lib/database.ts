...
export const updateActiveDownload = async (parentId: string, childId: string | null, updates: Partial<DownloadItem>): Promise<{
	success: boolean
}> => {
...
export const deleteActiveDownloadsVideoFromPlaylist = async (playlistId: string, videoId: string): Promise<{
	success: boolean
}> => {
	return await getElectron().db.deleteActiveDownloadsVideoFromPlaylist(playlistId, videoId);
};

export const deleteCompletedDownloadsVideoFromPlaylist = async (playlistId: string, videoId: string): Promise<{
	success: boolean
}> => {
	return await getElectron().db.deleteCompletedDownloadsVideoFromPlaylist(playlistId, videoId);
};

export const markDownloadItemAsFailed = async (downloadItem: DownloadItem): Promise<{
	success: boolean,
	error: Error | null
}> => {
	return await getElectron().db.markDownloadAsFailed(downloadItem);
}